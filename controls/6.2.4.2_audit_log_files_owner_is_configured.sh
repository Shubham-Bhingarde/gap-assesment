#!/usr/bin/env bash
# controls/6.2.4.2_audit_log_files_owner_is_configured.sh

execute_control() {
    local CONTROL_ID="6.2.4.2"
    local TITLE="Ensure audit log files owner is configured ((Automated)"
    local EXPECTED="Run the following script to verify audit log files are owned by the root user:
#!/usr/bin/env bash
{
l_output=\"\" l_output2=\"\"
...
potentially compromising its confidentiality."
    local ATTACK=""
    local REMEDIATION="Run the following command to configure the audit log files to be owned by the root
user:
