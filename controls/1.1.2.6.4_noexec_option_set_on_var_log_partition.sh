...
# mount -o remount /var/log"
    ...
    if [ -z "$MOUNT_CHECK" ]; then
        CURRENT="/var/log is not mounted, so noexec check is not applicable (fail by default or handle upstream)."
        RESULT="FAIL"
    else
        local OPTIONS=$(findmnt -kn -o OPTIONS "/var/log" 2>/dev/null)
        if echo "$OPTIONS" | grep -q "\bnoexec\b"; then
            CURRENT="/var/log is mounted with noexec option ($OPTIONS)."
            RESULT="PASS"
        else
            CURRENT="/var/log is mounted but missing noexec option ($OPTIONS)."
            RESULT="FAIL"
        fi
    fi
