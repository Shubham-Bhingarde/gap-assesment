#!/usr/bin/env bash
# controls/3.3.2.6_net_ipv6_conf_default_accept_source_route_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.2.6"
    local TITLE="Ensure net.ipv6.conf.default.accept_source_route is configured ((Automated)"
    local EXPECTED="- IF IPv6 is enabled on the system Verify net.ipv6.conf.default.accept_source_route is set to 0.
1. Run the following command to verify
net.ipv6.conf.default.accept_source_route is set to 0 in the running
configuration:
# sysctl net.ipv6.conf.default.accept_source_route
Verify output is:
net.ipv6.conf.default.accept_source_route = 0
2. Run the following script to verify
net.ipv6.conf.default.accept_source_route = 0 is set in a file being used
by systemd sysctl to configure
