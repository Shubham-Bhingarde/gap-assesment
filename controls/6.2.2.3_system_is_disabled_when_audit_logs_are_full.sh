#!/usr/bin/env bash
# controls/6.2.2.3_system_is_disabled_when_audit_logs_are_full.sh

execute_control() {
    local CONTROL_ID="6.2.2.3"
    local TITLE="Ensure system is disabled when audit logs are full ((Automated)"
    local EXPECTED="Run the following command and verify the disk_full_action is set to either halt or
single:
...
disk_error_action = <syslog|single|halt>"
    local RISK="Unknown"
    local DESC="The auditd daemon can be configured to halt the system or put the system in single
...
exceeds the benefit of the system's availability."
    local ATTACK="disk_full_action parameter:
•
•
...
log files."
    local REMEDIATION="Set one of the following parameters in /etc/audit/auditd.conf depending on your
