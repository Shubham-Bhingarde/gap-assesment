#!/usr/bin/env bash
# controls/2.4.1.8_access_to_etc_cron_d_is_configured.sh

execute_control() {
    local CONTROL_ID="2.4.1.8"
    local TITLE="Ensure access to /etc/cron.d is configured ((Automated)"
...
Access: (700/drwx------) Uid: ( 0/ root) Gid: ( 0/ root)"
    local RISK="Unknown"
    local DESC="The /etc/cron.d directory contains system cron jobs that need to run in a similar
