#!/usr/bin/env bash
# controls/1.6.1_etc_motd_is_configured.sh

execute_control() {
    local CONTROL_ID="1.6.1"
    local TITLE="Ensure /etc/motd is configured ((Automated)"
    local EXPECTED="Run the following script to verify MOTD files do not contain system information:
#!/usr/bin/env bash
{
l_output=\"\" l_output2=\"\"
a_files=()
...
have logged in."
    local ATTACK=""
    local REMEDIATION="Edit the file found in /etc/motd.d/* with the appropriate contents according to your
...
printf '%s\n' \"\${a_files[@]}\"
    ...
    local E_MATCH=$(grep -E -i "(\\v|\\r|\\m|\\s|$(grep '^ID=' /etc/os-release | cut -d= -f2 | sed -e 's/"//g'))" /etc/motd 2>/dev/null || true)

    if [ -n "$E_MATCH" ]; then
        CURRENT="/etc/motd contains OS information."
        RESULT="FAIL"
    else
        CURRENT="/etc/motd is properly configured."
        RESULT="PASS"
    fi
