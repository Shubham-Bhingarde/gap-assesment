#!/usr/bin/env bash
# controls/5.4.3.2_default_user_shell_timeout_is_configured.sh

execute_control() {
    local CONTROL_ID="5.4.3.2"
    local TITLE="Ensure default user shell timeout is configured ((Automated)"
    local EXPECTED="Run the following script to verify that TMOUT is configured to: include a timeout of no
...
one file."
    local RISK="Unknown"
    local DESC="TMOUT is an environmental setting that determines the timeout of a shell in seconds.
•
•
•
...
session and releases the resources associated with that session."
    local ATTACK=""
    local REMEDIATION="Review /etc/bashrc, /etc/profile, and all files ending in *.sh in the
