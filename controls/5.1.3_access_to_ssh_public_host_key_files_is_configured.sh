#!/usr/bin/env bash
# controls/5.1.3_access_to_ssh_public_host_key_files_is_configured.sh

execute_control() {
    local CONTROL_ID="5.1.3"
    local TITLE="Ensure access to SSH public host key files is configured ((Automated)"
    local EXPECTED="Run the following command and verify Access does not grant write or execute
...
\"\${a_output[@]}\" \"\"
    ...
    local RISK="Unknown"
    local DESC="An SSH public key is one of two files used in SSH public key authentication. In this
...
compromised."
    local ATTACK=""
    local REMEDIATION="Run the following script to set mode, ownership, and group on the public SSH host key
files:
#!/usr/bin/env bash
{
a_output=(); a_output2=()
