#!/usr/bin/env bash
# controls/1.1.1.3_hfs_kernel_module_is_not_available.sh
...
printf '%s\n' \"\" \"blacklist hfs\" >> /etc/modprobe.d/60-hfs.conf"
    ...
    if [ -z "$MOD_CHECK" ]; then
        CURRENT="hfs module is not available on the system."
        RESULT="PASS"
    else
        # Module is available, verify it is not loaded
        local LOAD_CHECK=$(lsmod | grep -P -- 'hfs' || true)

        # Verify it is not loadable
        local CONFIG_CHECK=$(modprobe --showconfig 2>/dev/null | grep -P -- '\b(install|blacklist)\h+hfs\b' || true)

        local HAS_BLACKLIST=false
        local HAS_INSTALL=false

        if echo "$CONFIG_CHECK" | grep -q "blacklist hfs"; then
            HAS_BLACKLIST=true
        fi

        if echo "$CONFIG_CHECK" | grep -qE "install hfs /bin/(false|true)"; then
            HAS_INSTALL=true
        fi
        ...
        if [ -n "$LOAD_CHECK" ]; then
            RESULT="FAIL"
            CURRENT="hfs module is currently loaded."
        elif [[ "$HAS_BLACKLIST" == "true" && "$HAS_INSTALL" == "true" ]]; then
            RESULT="PASS"
        else
            RESULT="FAIL"
            CURRENT="hfs module exists, is not loaded, but is not properly blacklisted and disabled."
        fi
    fi
