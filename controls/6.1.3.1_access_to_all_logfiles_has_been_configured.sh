#!/usr/bin/env bash
# controls/6.1.3.1_access_to_all_logfiles_has_been_configured.sh

execute_control() {
    local CONTROL_ID="6.1.3.1"
    local TITLE="Ensure access to all logfiles has been configured ((Automated)"
    local EXPECTED="Run the following script to verify that files in /var/log/ have appropriate permissions
and ownership:
#!/usr/bin/env bash
{
a_output=(); a_output2=()
...
a_out2+=(\"
o Mode: \\\"\$l_mode\\\" should be \\\"\$maxperm\\\" or more restrictive\")
[[ ! \"\$l_user\" =~ \$l_auser ]] && \
a_out2+=(\"
o Owned by: \\\"\$l_user\\\" and should be owned by \\\"\${l_auser//|/ or }\\\"\")
[[ ! \"\$l_group\" =~ \$l_agroup ]] && \
a_out2+=(\"
o Group owned by: \\\"\$l_group\\\" and should be group owned by
\\\"\${l_agroup//|/ or }\\\"\")
...
\"\${a_output2[@]}\" \"\"
    ...
    local RISK="Unknown"
    local DESC="Log files contain information from many services on the the local system, or in the event
...
protected and that only the appropriate users / groups have access to them."
    local ATTACK=""
    local REMEDIATION="Run the following script to update permissions and ownership on files in /var/log.
...
o Owned by: \\\"\$l_user\\\" and should be owned by \\\"\${l_auser//|/ or }\\\"\" \"
x
Changing ownership to: \\\"\$l_fix_account\\\"\")
chown \"\$l_fix_account\" \"\$l_fname\"
fi
if [[ ! \"\$l_group\" =~ \$l_agroup ]]; then
a_out2+=(\"
o Group owned by: \\\"\$l_group\\\" and should be group owned by \\\"\${l_agroup//|/ or
}\\\"\" \"
...
file_test_fix ;;
secure | auth.log | syslog | messages)
perm_mask='0137' l_rperms=\"u-x,g-wx,o-rwx\" l_auser=\"(root|syslog)\"
l_agroup=\"(root|adm)\"
f_file_test_fix ;;
SSSD | sssd)
...
f_file_test_fix ;;
*.journal | *.journal~)
perm_mask='0137' l_rperms=\"u-x,g-wx,o-rwx\" l_auser=\"root\" l_agroup=\"(root|systemdjournal)\"
f_file_test_fix ;;
*)
perm_mask='0137' l_rperms=\"u-x,g-wx,o-rwx\" l_auser=\"(root|syslog)\"
l_agroup=\"(root|adm)\"
...
a_output+=(\" - All files in \\\"/var/log/\\\" have appropriate permissions and ownership\")
printf '\n%s' \"- All files in \\\"/var/log/\\\" have appropriate permissions and ownership\" \" o No
changes required\" \"\"
else
printf '\n%s' \"\${a_output2[@]}\" \"\"
fi
