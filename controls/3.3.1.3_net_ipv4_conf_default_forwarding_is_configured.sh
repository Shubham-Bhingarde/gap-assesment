#!/usr/bin/env bash
# controls/3.3.1.3_net_ipv4_conf_default_forwarding_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.1.3"
    local TITLE="Ensure net.ipv4.conf.default.forwarding is configured ((Automated)"
    local EXPECTED="Verify net.ipv4.conf.default.forwarding is set to 0.
1. Run the following command to verify net.ipv4.conf.default.forwarding is
set to 0 in the running configuration:
# sysctl net.ipv4.conf.default.forwarding
Verify output is:
net.ipv4.conf.default.forwarding = 0
2. Run the following script to verify net.ipv4.conf.default.forwarding = 0 is
...
Example output:
- \"net.ipv4.conf.default.forwarding = 0\" is set in: \"/etc/sysctl.d/60ipv4_sysctl.conf\"
Note:
•
•
•
