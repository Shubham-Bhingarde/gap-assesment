#!/usr/bin/env bash
# controls/3.3.2.4_net_ipv6_conf_default_accept_redirects_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.2.4"
    local TITLE="Ensure net.ipv6.conf.default.accept_redirects is configured ((Automated)"
    local EXPECTED="- IF IPv6 is enabled on the system Verify net.ipv6.conf.default.accept_redirects is set to 0.
1. Run the following command to verify
net.ipv6.conf.default.accept_redirects is set to 0 in the running
configuration:
# sysctl net.ipv6.conf.default.accept_redirects
Verify output is:
net.ipv6.conf.default.accept_redirects = 0
2. Run the following script to verify net.ipv6.conf.default.accept_redirects
...
Example output:
- \"net.ipv6.conf.default.accept_redirects = 0\" is set in: \"/etc/sysctl.d/60ipv6_sysctl.conf\"
Note:
•
•
•
