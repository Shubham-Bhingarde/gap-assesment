...
/etc/fstab as appropriate."
    ...
    local SYSTEMD_CHECK=$(systemctl is-enabled $(systemd-escape --suffix=mount "/var/log") 2>/dev/null || echo "")

    if [ -n "$MOUNT_CHECK" ]; then
        CURRENT="/var/log is mounted. Systemd status: ${SYSTEMD_CHECK:-unknown}"
        RESULT="PASS"
    else
        CURRENT="/var/log is not mounted as a separate partition or tmpfs."
        RESULT="FAIL"
    fi
