...
/etc/issue.net"
    ...
    local E_MATCH=$(grep -E -i "(\\v|\\r|\\m|\\s|$(grep '^ID=' /etc/os-release | cut -d= -f2 | sed -e 's/"//g'))" /etc/issue.net 2>/dev/null || true)

    if [ -n "$E_MATCH" ]; then
        CURRENT="/etc/issue.net contains OS information."
        RESULT="FAIL"
    else
        CURRENT="/etc/issue.net is properly configured."
        RESULT="PASS"
    fi
