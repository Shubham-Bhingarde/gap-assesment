#!/usr/bin/env bash
# controls/3.3.1.4_net_ipv4_conf_all_send_redirects_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.1.4"
    local TITLE="Ensure net.ipv4.conf.all.send_redirects is configured ((Automated)"
    local EXPECTED="Verify net.ipv4.conf.all.send_redirects is set to 0.
1. Run the following command to verify net.ipv4.conf.all.send_redirects is
...
used by systemd sysctl to configure net.ipv4.conf.all.send_redirects:
#!/usr/bin/env bash
{
...
Example output:
- \"net.ipv4.conf.all.send_redirects = 0\" is set in: \"/etc/sysctl.d/60ipv4_sysctl.conf\"
Note:
•
•
•
