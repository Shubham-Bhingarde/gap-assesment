#!/usr/bin/env bash
# controls/5.3.3.4.2_pam_unix_does_not_include_remember.sh

execute_control() {
    local CONTROL_ID="5.3.3.4.2"
    local TITLE="Ensure pam_unix does not include remember ((Automated)"
    local EXPECTED="Run the following command to verify that the remember argument is not set on the
pam_unix.so module:
# grep -PHs -'^\h*^\h*[^#\n\r]+\h+pam_unix\.so\h+([^#\n\r]+\h+)?remember=\d+\b'
/etc/pam.d/common-{password,auth,account,session,session-noninteractive}
Nothing should be returned"
    local RISK="Unknown"
    local DESC="The remember=n argument saves the last n passwords for each user in
...
[success=end default=ignore]
pam_unix.so obscure yescrypt remember=5 # **<remove remember=<N>**
