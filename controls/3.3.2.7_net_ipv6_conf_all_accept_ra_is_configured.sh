...
execute_control() {
    ...
    local EXPECTED="- IF IPv6 is enabled on the system Verify net.ipv6.conf.all.accept_ra is set to 0.
1. Run the following command to verify net.ipv6.conf.all.accept_ra is set to 0
in the running configuration:
# sysctl net.ipv6.conf.all.accept_ra
Verify output is:
net.ipv6.conf.all.accept_ra = 0
2. Run the following script to verify net.ipv6.conf.all.accept_ra = 0 is set in a
file being used by systemd sysctl to configure net.ipv6.conf.all.accept_ra:
#!/usr/bin/env bash
{
...
Example output:
- \"net.ipv6.conf.all.accept_ra = 0\" is set in: \"/etc/sysctl.d/60ipv6_sysctl.conf\"
Note:
•
•
•
...
file that is ordered later)--"
    local RISK="Unknown"
    local DESC="Routers periodically multicast Router Advertisement messages to announce their
...
# printf '%s\n' \"\" \"net.ipv6.conf.all.accept_ra = 0\" >> /etc/sysctl.d/60ipv6_sysctl.conf
