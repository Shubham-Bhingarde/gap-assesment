...
0 0"
    ...
    local SYSTEMD_CHECK=$(systemctl is-enabled $(systemd-escape --suffix=mount "/dev/shm") 2>/dev/null || echo "")

    if [ -n "$MOUNT_CHECK" ]; then
        CURRENT="/dev/shm is mounted. Systemd status: ${SYSTEMD_CHECK:-unknown}"
        RESULT="PASS"
    else
        CURRENT="/dev/shm is not mounted as a separate partition or tmpfs."
        RESULT="FAIL"
    fi
