...
# printf '%s\n' \"\" \"blacklist usb_storage\" >> /etc/modprobe.d/60-usbstorage.conf"
    ...
    local MOD_CHECK=$(
        l_mod_name="usb-storage" l_mod_type="fs"
    ...
    if [ -z "$MOD_CHECK" ]; then
        CURRENT="usb-storage module is not available on the system."
        RESULT="PASS"
    else
        # Module is available, verify it is not loaded
        local LOAD_CHECK=$(lsmod | grep -P -- 'usb(_|-)storage' || true)

        # Verify it is not loadable
        local CONFIG_CHECK=$(modprobe --showconfig 2>/dev/null | grep -P -- '\b(install|blacklist)\h+usb_storage\b' || true)

        local HAS_BLACKLIST=false
        local HAS_INSTALL=false

        if echo "$CONFIG_CHECK" | grep -q "blacklist usb_storage"; then
            HAS_BLACKLIST=true
        fi

        if echo "$CONFIG_CHECK" | grep -qE "install usb_storage /bin/(false|true)"; then
            HAS_INSTALL=true
        fi
        ...
        if [ -n "$LOAD_CHECK" ]; then
            RESULT="FAIL"
            CURRENT="usb-storage module is currently loaded."
        elif [[ "$HAS_BLACKLIST" == "true" && "$HAS_INSTALL" == "true" ]]; then
            RESULT="PASS"
        else
            RESULT="FAIL"
            CURRENT="usb-storage module exists, is not loaded, but is not properly blacklisted and disabled."
        fi
    fi
