#!/usr/bin/env bash
# controls/6.2.3.10_successful_file_system_mounts_are_collected.sh

execute_control() {
    local CONTROL_ID="6.2.3.10"
    local TITLE="Ensure successful file system mounts are collected ((Automated)"
...
-a always,exit -F arch=b32 -S mount -F auid>=1000 -F auid!=-1 -F key=mounts"
    local RISK="Unknown"
    local DESC="Monitor the use of the mount system call. The mount (and umount ) system call controls
