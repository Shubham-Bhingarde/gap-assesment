#!/usr/bin/env bash
# controls/2.4.2.1_access_to_at_is_configured.sh

execute_control() {
    local CONTROL_ID="2.4.2.1"
    local TITLE="Ensure access to at is configured ((Automated)"
    local EXPECTED="- IF - at is installed on the system:
...
daemon or root"
    local RISK="Unknown"
    local DESC="at allows fairly complex time specifications, extending the POSIX.2 standard. It accepts
...
ID to the system and forget to add it to the deny files."
    local ATTACK=""
    local REMEDIATION="- IF - at is installed on the system:
