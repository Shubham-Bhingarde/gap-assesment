#!/usr/bin/env bash
# controls/3.3.1.18_net_ipv4_tcp_syncookies_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.1.18"
    local TITLE="Ensure net.ipv4.tcp_syncookies is configured ((Automated)"
    local EXPECTED="Verify net.ipv4.tcp_syncookies is set to 1.
1. Run the following command to verify net.ipv4.tcp_syncookies is set to 1 in
the running configuration:
# sysctl net.ipv4.tcp_syncookies
Verify output is:
net.ipv4.tcp_syncookies = 1
2. Run the following script to verify net.ipv4.tcp_syncookies = 1 is set in a file
being used by systemd sysctl to configure net.ipv4.tcp_syncookies:
#!/usr/bin/env bash
{
...
Example output:
- \"net.ipv4.tcp_syncookies = 1\" is set in: \"/etc/sysctl.d/60ipv4_sysctl.conf\"
Note:
•
•
•
...
file that is ordered later)--"
    local RISK="Unknown"
    local DESC="When tcp_syncookies is set, the kernel will handle TCP SYN packets normally until
...
# printf '%s\n' \"\" \"net.ipv4.tcp_syncookies = 1\" >> /etc/sysctl.d/60ipv4_sysctl.conf
