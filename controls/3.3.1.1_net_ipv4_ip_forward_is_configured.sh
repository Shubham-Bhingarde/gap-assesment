#!/usr/bin/env bash
# controls/3.3.1.1_net_ipv4_ip_forward_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.1.1"
    local TITLE="Ensure net.ipv4.ip_forward is configured ((Automated)"
    local EXPECTED="Verify net.ipv4.ip_forward is set to 0.
1. Run the following command to verify net.ipv4.ip_forward is set to 0 in the
running configuration:
# sysctl net.ipv4.ip_forward
Verify output is:
net.ipv4.ip_forward = 0
2. Run the following script to verify net.ipv4.ip_forward = 0 is set in a file being
used by systemd sysctl to configure net.ipv4.ip_forward:
#!/usr/bin/env bash
{
...
file that is ordered later)--"
    local RISK="Unknown"
    local DESC="The net.ipv4.ip_forward flag tells the system whether it can forward IPv4 packets or
not.
Note:
•
•
If this system is a router this recommendation is not applicable.
If net.ipv4.conf.all.forwarding=0 and
net.ipv4.conf.default.forwarding=0 are configured, this recommendation
may be skipped.
