#!/usr/bin/env bash
# controls/3.3.2.8_net_ipv6_conf_default_accept_ra_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.2.8"
    local TITLE="Ensure net.ipv6.conf.default.accept_ra is configured ((Automated)"
    local EXPECTED="- IF IPv6 is enabled on the system Verify net.ipv6.conf.default.accept_ra is set to 0.
1. Run the following command to verify net.ipv6.conf.default.accept_ra is
set to 0 in the running configuration:
# sysctl net.ipv6.conf.default.accept_ra
Verify output is:
net.ipv6.conf.default.accept_ra = 0
2. Run the following script to verify net.ipv6.conf.default.accept_ra = 0 is
...
Example output:
- \"net.ipv6.conf.default.accept_ra = 0\" is set in: \"/etc/sysctl.d/60ipv6_sysctl.conf\"
Note:
•
•
•
...
file that is ordered later)--"
    local RISK="Unknown"
    local DESC="Routers periodically Multicast Router Advertisement (RA) messages to announce their
...
# printf '%s\n' \"\" \"net.ipv6.conf.default.accept_ra = 0\" >> /etc/sysctl.d/60ipv6_sysctl.conf
