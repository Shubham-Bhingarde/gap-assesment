...
# apt purge telnet"
    ...
    local PKG_CHECK=$(dpkg-query -W -f='${Status}' "telnet" 2>/dev/null | grep -c "install ok installed" || true)

    if [ "$PKG_CHECK" -eq 0 ]; then
        CURRENT="telnet package is not installed."
        RESULT="PASS"
    else
        CURRENT="telnet package is installed."
        RESULT="FAIL"
    fi
