#!/usr/bin/env bash
# controls/3.3.2.1_net_ipv6_conf_all_forwarding_is_configured.sh

execute_control() {
    local CONTROL_ID="3.3.2.1"
    local TITLE="Ensure net.ipv6.conf.all.forwarding is configured ((Automated)"
    local EXPECTED="- IF IPv6 is enabled on the system Verify net.ipv6.conf.all.forwarding is set to 0.
1. Run the following command to verify net.ipv6.conf.all.forwarding is set to
0 in the running configuration:
# sysctl net.ipv6.conf.all.forwarding
Verify output is:
net.ipv6.conf.all.forwarding = 0
2. Run the following script to verify net.ipv6.conf.all.forwarding = 0 is set in
...
Example output:
- \"net.ipv6.conf.all.forwarding = 0\" is set in: \"/etc/sysctl.d/60ipv6_sysctl.conf\"
Note:
•
•
•
...
# printf '%s\n' \"\" \"net.ipv6.conf.all.forwarding = 0\" >> /etc/sysctl.d/60ipv6_sysctl.conf
