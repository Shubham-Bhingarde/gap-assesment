...
execute_control() {
    ...
    local EXPECTED="Run the following command and verify ClientAliveInterval and
...
address), lport (local port number), and rdomain (routing domain)."
    local RISK="Unknown"
    local DESC="Note: To clarify, the two settings described below are only meant for idle connections
...
is left at the default, unresponsive SSH clients will be disconnected after
approximately 45 seconds. Setting a zero ClientAliveCountMax disables
...
timeouts and apply as appropriate."
    local ATTACK=""
    local REMEDIATION="Edit the /etc/ssh/sshd_config file to set the ClientAliveInterval and
