#!/usr/bin/env bash
# controls/1.1.2.6.2_nodev_option_set_on_var_log_partition.sh

execute_control() {
    local CONTROL_ID="1.1.2.6.2"
    local TITLE="Ensure nodev option set on /var/log partition ((Automated)"
    local EXPECTED="- IF - a separate partition exists for /var/log, verify that the nodev option is set.
...
# mount -o remount /var/log"
    ...
    if [ -z "$MOUNT_CHECK" ]; then
        CURRENT="/var/log is not mounted, so nodev check is not applicable (fail by default or handle upstream)."
        RESULT="FAIL"
    else
        local OPTIONS=$(findmnt -kn -o OPTIONS "/var/log" 2>/dev/null)
        if echo "$OPTIONS" | grep -q "\bnodev\b"; then
            CURRENT="/var/log is mounted with nodev option ($OPTIONS)."
            RESULT="PASS"
        else
            CURRENT="/var/log is mounted but missing nodev option ($OPTIONS)."
            RESULT="FAIL"
        fi
    fi
